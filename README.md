# OnlineMarketPlace

## Description
This is an onlineMarketplace project (ByteBazaar) inspired by platforms like OLX. It allows users to buy and sell various items within different categories. The project is built using Python and Django for the backend, and Tailwind CSS for the frontend.

<img src="./preview.jpg" width = "100%">


## Features
- User authentication and authorization
- User profiles
- Listing items for sale
- Searching and filtering items by category, price, etc.
- Messaging between buyers and sellers
- Admin panel for managing users, listings, and categories

## Technologies Used
- Python
- Django
- Tailwind CSS

## Getting Started
To get started with this project, follow these steps:

1. Clone this repository to your local machine.
2. Create a virtual environment for the project and activate it.
3. Apply migrations to set up the database using `python manage.py migrate`.
4. Create a superuser for accessing the admin panel using `python manage.py createsuperuser`.
5. Run the development server using `python manage.py runserver`.
6. Access the website at `http://localhost:8000`.

## Contributing
Contributions are welcome! If you have any ideas for new features, bug fixes, or improvements, feel free to open an issue or submit a pull request.

## License
This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

